...
const count = (target, input) => input.reduce((a, b) => a + b.reduce((a, b) => a + (b===target),0),0);
  // Solution code here...
...
const totalSum = (input) => input.reduce((a, b) => a + b.reduce((a, b) => a + b, 0), 0);
  // Solution code here...
...
const divisibleByFiveTwoToThePower = (input) => input.map(a => a.filter(a => (typeof a === 'number' && a % 5 === 0)).map(a => 2 ** a));
  // Solution code here...
...
let findShortest = (data) => {
  // Solution code here...
  const shortest = data.reduce((a, b) => (parseInt(a.height) < parseInt(b.height)) ? a : b).name;
  return shortest;
};
